...
module.exports = class CurrencyUser {
    constructor(user, options) {
        ...
        this.storage.userTable = options.userTable;
        this.storage.itemTable = options.itemTable;
    ...
    async #_getUser({ user }) {
        const data = await this.userTable.get(user);
    ...
    async #_saveUser({ user }) {
        if(!user) user = this;
        await this.userTable.set(user.user, user);
        return user;
    };

    async #_getShop() {
        const data = await this.itemTable.get('shop');
        return data;
    };
};