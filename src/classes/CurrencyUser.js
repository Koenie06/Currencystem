...
module.exports = class CurrencyUser {
    ...
    async #_getUser({ user }) {
        this.options.connection.query('SHOW TABLES LIKE "users"', (error, results) => {
            ...
            this.options.connection.query('SELECT * FROM users WHERE user = ?', [user], (error, results) => {
                ...
                if(results.length === 0) {
                    this.options.connection.query('INSERT INTO `users` (`user`, `wallet`, `bank`, `items`, `lastWork`, `lastBeg`, `lastHourly`, `lastDaily`, `lastWeekly`, `lastMonthly`, `lastYearly`) VALUES (?,?,?,?)', [user, 0, 0, [], null, null, null, null, null, null, null], (error) => { throw error; });
    ...
    async #_saveUser({ user }) {
        if(!user) user = this;
        this.options.connection.query('UPDATE users SET wallet = ?, bank = ?, items = ?, lastWork = ?, lastBeg = ?, lastHourly = ?, lastDaily = ?, lastWeekly = ?, lastMonthly = ?, lastYearly = ? WHERE user = ?', [user.wallet, user.bank, JSON.stringify(user.items), user.lastWork, user.lastBeg, user.lastHourly, user.lastDaily, user.lastWeekly, user.lastMonthly, user.lastYearly, user.user], (error) => { throw error; });
        return user;
    };
