...
module.exports = class CurrencyUser {
    constructor(user, options) {
        ...
        if(!this.options.storage.users) throw new Error('Users storage is not initialized.');
        if(!this.options.storage.items) throw new Error('Items storage is not initialized.');
    ...
    async addMoney({ amount, place }) {
        ...
        return new CurrencyUser(newUser, this.options);
    };
    ...
    async deposit(amount) {
        ...
        return new CurrencyUser(addData, this.options);
    };
     ...
     */
     async gamble({ amount, place }) {
        ...
        if(amount <= 0) throw new Error('Amount must be positive or higher than 0.');
        if(typeof place !== 'string' || !place || !(['wallet', 'bank'].includes(place))) throw new Error('Place must be a string called \'wallet\' or \'bank\'.');
        if(amount > this[place]) return { error: true, message: 'User does not have enough money in his wallet/bank.' };
        ...
        if(random === 0) {
            ...
            if(removeData.error) return removeData;
            return {
                user: new CurrencyUser(removeData, this.options),
                lost: true
            };
        } else {
            const addData = await this.addMoney({ amount: amount, place: place });
            if(addData.error) return addData;
            return {
                user: new CurrencyUser(addData, this.options),
                won: true
            };
        };
    };
     ...
     */
     async rob({ victim, minimum, maximum, succes }) {
        ...
        if(luck <= succes) {
            ...
            const removeUser = await this.#_saveUser(victimObject);
            ...
            const addUser = await this.#_saveUser(victimObject);
            if(addUser.error) return addUser;
            return {
                lost: true,
                amount: random,
    ...
    async work({ place, minimum, maximum, cooldown }) {
        ...
        return {
            earned: random,
            user: new CurrencyUser(newUser, this.options),
        };
    };
    ...
    async transferMoney({ addTo, amount, place }) {
        ...
        if(addData.error) return addData;
        return {
            removeFrom: new CurrencyUser(removeData, this.options),
            addTo: new CurrencyUser(addData, this.options)
        };
    };
    ...
    async #_getUser({ user }) {
        ...
        return userStorage[0] ? userStorage[0] : { user: user, wallet: this.options.defaultWallet || 0, bank: this.options.defaultBank || 0, items: [], lastWork: null, lastBeg: null };
    };

    async #_saveUser(user = this) {
        const storageContent = await readFile(this.options.storage.users, { encoding: 'utf-8' });
        if (!storageContent.trim().startsWith('[') || !storageContent.trim().endsWith(']')) {
            console.log(storageContent);
            throw new SyntaxError('The storage file is not properly formatted (does not contain an array).');
        ...
        userIndex >= 0 ? storage[userIndex] = user : storage.push(user);
