...
module.exports.CurrencyManager = class CurrencyManager {
     ...
     * @param {Object=} options.storage The path to the storage files.
     * @param {string=} options.storage.users The path to the users storage file.
     * @param {string=} options.storage.items The path to the shop/items storage file.
     */
    constructor(options) {
     ...
     * @param {'wallet'|'bank'} options.place The place to store the money in.
     ...
     * @param {'wallet'|'bank'} options.place The place to remove the money from.
     * @returns {Promise<CurrencyUser[]>} All the users and the amount of money they have.
     */
     ...
     * @param {Function=} filter A filter function to filter shop items (optional).
    ...
    async #_getUser({ user }) {
        ...
        return userStorage[0] ? userStorage[0] : { user: user, wallet: 0, bank: 0, items: [], lastWork: null, lastBeg: null, lastHourly: null, lastDaily: null, lastWeekly: null, lastMonthly: null, lastYearly: null };	
    };

    async #_getAllUsers({ filter = false }) {
        const storageContent = await readFile(this.options.storage.users, { encoding: 'utf-8' });
        if (!storageContent.trim().startsWith('[') || !storageContent.trim().endsWith(']')) {
            console.log(storageContent);
            throw new SyntaxError('The storage file is not properly formatted (does not contain an array).');
        ...
        const storage = JSON.parse(storageContent);
        if(filter) return storage.filter(filter)
        return storage;
    }
