...
module.exports.CurrencyManager = class CurrencyManager {
     ...
     * @param {Object} options.storage The path to the custom sql file, else use the default.
     */
    constructor(options) {
        this.options = options;
        
        if(typeof this.options.storage !== 'string' || !this.options.storage) this.options.storage = './currency.sqlite';
        
        this.db = new QuickDB({ filePath: this.options.storage })
        this.options.userTable = this.db.table('users');
        this.options.itemTable = this.db.table('items');

        console.log("[Currencystem] - Connected to the sql database.");
    };
    ...
    async #_getUser({ user }) {
        const data = await this.options.userTable.get(user);
        
        data ? data : { user: user, wallet: 0, bank: 0, items: [], lastWork: null, lastBeg: null, lastHourly: null, lastDaily: null, lastWeekly: null, lastMonthly: null, lastYearly: null };
    };

    async #_getAllUsers({ filter }) {
        const data = await this.options.userTable.all();
        if(filter) return data.filter(filter);
        return data;
    }

    async #_getShop({ filter }) {
        const data = await this.options.itemTable.get('shop');
        if(filter) return data.filter(filter);
        return data;
    };

    async #_saveShop(items) {
        await this.options.itemTable.set('shop', items);
        return items;
    }
};