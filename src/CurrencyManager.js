...
module.exports.CurrencyManager = class CurrencyManager {
     ...
     * @param {Object} options.user The username of the mysql.
     * @param {string} options.password The password to the mysql.
     * @param {string} options.host The host IP to the mysql.
     * @param {string} options.database The name of the database to use.
     * @param {number} options.port The port to the mysql.
     */
    constructor(options) {
        this.options = options;
        
        if(typeof options.user !== 'string' || !options.user) throw new Error('Database user must be a string.');
        if(typeof options.password !== 'string' || !options.password) throw new Error('Database password must be a string.');
        if(typeof options.host !== 'string' || !options.host) throw new Error('Database host must be a string.');
        if(typeof options.database !== 'string' || !options.database) throw new Error('Database name must be a string.');
        if(typeof options.port !== 'number') throw new Error('Database port must be a string.');
        
        this.options.connection = mysql.createConnection({
            ...
            console.log("[Currencystem] - Connected to the mysql database.");
        });
    };
    ...
    async #_getUser({ user }) {
        this.options.connection.query('SHOW TABLES LIKE "users"', (error, results) => {
            if(error) throw new Error("Table 'users' does not exist.");
            this.options.connection.query('SELECT * FROM users WHERE user = ?', [user], (error, results) => {
                if(error) throw error;
                if(results.length === 0) {
                    this.options.connection.query('INSERT INTO `users` (`user`, `wallet`, `bank`, `items`, `lastWork`, `lastBeg`, `lastHourly`, `lastDaily`, `lastWeekly`, `lastMonthly`, `lastYearly`) VALUES (?,?,?,?)', [user, 0, 0, JSON.stringify([]), null, null, null, null, null, null, null], (error) => { throw error; });
                    return { user: user, wallet: 0, bank: 0, items: [], lastWork: null, lastBeg: null, lastHourly: null, lastDaily: null, lastWeekly: null, lastMonthly: null, lastYearly: null };
                }
                return results[0];
            }).catch(error => {
                throw error;
    ...
    async #_getAllUsers({ filter }) {
        this.options.connection.query('SHOW TABLES LIKE "users"', (error, results) => {
            if(error) throw new Error("Table 'users' does not exist.");
            this.options.connection.query('SELECT * FROM users', (error, results) => {
                if(error) throw error;
                if(filter) return results.filter(filter).catch(() => { return []; });
                return results;
            }).catch(error => {
                throw error;
            });
    ...
    async #_getShop({ filter }) {
        this.options.connection.query('SHOW TABLES LIKE "shop"', (error, results) => {
            if(error) throw new Error("Table 'shop' does not exist.");
            this.options.connection.query('SELECT * FROM shop', (error, results) => {
                if(error) throw error;
                if(filter) return results.filter(filter).catch(() => { return []; });
                return results;
            }).catch(error => {
    ...
    async #_saveShop(items) {
        this.options.connection.query('SHOW TABLES LIKE "shop"', (error, results) => {
            if(error) throw new Error("Table 'shop' does not exist.");
            this.options.connection.query('DELETE FROM "shop"', (error) => {
                if(error) throw error;
                let sqlValues = "";
                items.forEach(item => {
                    sqlValues += `('${item.name}', '${item.description}', ${item.price}, '${item.use.toString()}', ${Date.now()}),`;
                })
                this.options.connection.query(`INSERT INTO "shop" ("name", "description", "price", "use", "createdAt") VALUES ${sqlValues}`, (error) => {
                    if(error) throw error;
                    return items;
                })
            })
